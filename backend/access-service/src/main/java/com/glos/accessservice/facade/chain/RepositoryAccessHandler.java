...
public class RepositoryAccessHandler extends AccessHandler {
    ...
    public RepositoryAccessHandler(RepositoryApiClient repositoryApiClient) {
    ...
    public boolean check(AccessRequest request) {
        ...
        if (!data.containsKey("accessTypes") && node.getType() == NodeType.REPOSITORY) {
            final ResponseEntity<RepositoryDTO> response = repositoryApiClient.getRepositoryByRootFullName(path.getPath());
